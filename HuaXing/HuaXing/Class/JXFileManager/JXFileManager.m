...
-(void)sandBoxGetInfo:(Class)cls forKey:(NSString *)key operateBlock:(JXFileManagerOperateBlock)operateBlock {
    id rlt = [self sandBoxGetInfo:cls forKey:key];
    operateBlock ? operateBlock(TRUE,rlt) : nil;
}
...
-(void)unarchiveObjWithFileKey:(NSString *)key operateBlock:(JXFileManagerOperateBlock)operateBlock {
    id rlt = [self unarchiveObjWithFileKey:key];
    operateBlock ? operateBlock(TRUE,rlt) : nil;
}
...
-(BOOL)clearArchivedRootObjWithKey:(NSString *)key {
    NSString *archivePath = [[self cacheCommonDataCachePath] stringByAppendingString:key];
    BOOL rlt = [self removeItemAtFilePath:archivePath];
    return rlt;
}
